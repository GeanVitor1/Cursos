  ...
  etapas: [
    {
      ...
      blocos: [
        ...
        { tipo: 'texto', texto: 'Agora responda mentalmente: como você faria uma lista de produtos? Como enviaria um produto inteiro para uma função? Como saber que `preco1` pertence a `nome1`?' },
    ...
    },
    {
      ...
      atividade: {
        ...
        dicas: ['Pense em como você entregaria um produto inteiro para uma função.', 'O que mantém nome, preço e estoque unidos?'],
        explicacao: 'Quando os dados da mesma entidade ficam espalhados, qualquer operação (listar, ordenar, salvar) vira um malabarismo. A classe resolve isso.',
    ...
    },
    {
      ...
      blocos: [
        ...
        { tipo: 'trabalho', texto: 'Entidades como `Produto` são exatamente as classes que o Entity Framework mapeia para tabelas. O que você aprendeu aqui é o ponto de partida da trilha de dados.', fonte: '💼 No trabalho' }
      ]
    },
    {
      tipo: 'atividade',
    ...
    },
    {
      ...
      atividade: {
        ...
        tipo: 'write-code',
        dimensao: 'construcao',
        enunciado: 'Escreva uma classe `Cliente` com as propriedades `Id` (int), `Nome` (string) e `Email` (string).',
        ...
        respostasAceitas: ['public class Cliente { public int Id { get; set; } public string Nome { get; set; } public string Email { get; set; } }'],
        dicas: ['Uma propriedade segue o padrão: tipo + Nome + { get; set; }.', 'Use public antes de cada propriedade.'],
        explicacao: 'Cada propriedade precisa de tipo, nome e `{ get; set; }`. Essa é a base das entidades que você vai mapear no Entity Framework.',
    ...
    },
    {
      ...
      atividade: {
        ...
        tipo: 'scenario',
        dimensao: 'aplicacao',
        cena: 'Você entrou em um projeto e encontrou a função `SalvarProduto` recebendo 9 parâmetros soltos: nome, preco, estoque, categoria, fornecedor, peso, altura, largura, ativo. Cada vez que alguém chama a função, precisa lembrar a ordem exata.',
        enunciado: 'Qual mudança resolve a raiz do problema?',
        opcoes: [
          'Criar uma classe Produto e passar um único objeto para a função',
          'Escrever a lista de parâmetros em um comentário',
          'Criar mais uma função com outros 9 parâmetros',
          'Pedir para o compilador reordenar os parâmetros automaticamente'
        ],
        correta: 0,
        feedbackErro: {
          1: 'Comentário não valida nada: na próxima chamada o erro volta.',
          2: 'Outra função com 9 parâmetros repete o mesmo problema em dobro.',
          3: 'O compilador não adivinha a intenção: ele só avisa quando os tipos não batem.'
        },
        dicas: ['O problema é agrupar informações do mesmo conceito.', 'Foi exatamente para isso que você aprendeu classe e objeto.'],
        explicacao: 'Com a classe Produto, a função passa a receber uma coisa só, com significado claro. Esse é o uso mais comum de classes no backend.',
